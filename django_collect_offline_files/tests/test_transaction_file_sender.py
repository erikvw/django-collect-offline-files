...
class TestTransactionFileSender(TestCase):
    ...
    def test_send_custom_paths(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(archive_path, src_filename)))
    ...
    def test_send_default_paths(self):
        ...
        tx_file_sender.send(filenames=[src_filename])
        self.assertTrue(
            os.path.exists(os.path.join(app_config.incoming_folder, src_filename))
        )
        self.assertTrue(
            os.path.exists(os.path.join(app_config.archive_folder, src_filename))
        )
    ...
    def test_send_update_history(self):
        ...
        _, src = tempfile.mkstemp(text=True, dir=app_config.outgoing_folder)
        src_filename = os.path.basename(src)
        ExportedTransactionFileHistory.objects.create(filename=src_filename, sent=False)
        tx_file_sender = TransactionFileSender(
            history_model=ExportedTransactionFileHistory,
            src_path=app_config.outgoing_folder,
            dst_tmp=app_config.tmp_folder,
        ...
        try:
            ExportedTransactionFileHistory.objects.get(
                filename=src_filename, sent=True, sent_datetime__isnull=False
            )
        except ExportedTransactionFileHistory.DoesNotExist:
            self.fail("ExportedTransactionFileHistory.DoesNotExist unexpectedly raised")
    ...
    def test_transaction_file_sender_username(self):
        ...
        self.assertRaises(
            TransactionFileSenderError, tx_file_sender.send, filenames=[src_filename]
        )
