...
class TestConnector(TestCase):
    ...
    def test_username(self):
        ...
        options = dict(
            remote_host=remote_host, trusted_host=False, username="bob", timeout=1
        )
        trusted_host = True
        ...
        try:
            ...
            self.assertEqual(str(e.__cause__), f"Authentication failed.")
        else:
    ...
    def test_timeout_nottrusted(self):
        remote_host = "127.0.0.1"
        ssh_client = SSHClient(remote_host=remote_host, trusted_host=False, timeout=1)
        try:
            with ssh_client.connect() as c:
                c.close()
        except SSHClientError as e:
    ...
    def test_timeout_trusted(self):
        ssh_client = SSHClient(remote_host="127.0.0.0", trusted_host=True, timeout=1)
        try:
            with ssh_client.connect() as c:
                c.close()
        except SSHClientError as e:
    ...
    def test_timeout_not_trusted(self):
        ssh_client = SSHClient(remote_host="localhost", username="thing1", timeout=1)
        try:
            with ssh_client.connect() as c:
                c.close()
        except SSHClientError as e:
