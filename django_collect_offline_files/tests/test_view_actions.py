...
class TestActionHandler(TestCase):
    ...
    def test_invalid_action(self):
        ...
        self.assertRaises(ActionHandlerError, action_handler.action, label="blahblah")
    ...
    def test_export_batch_error(self):
        ...
        self.assertRaises(ActionHandlerError, action_handler.action, label=EXPORT_BATCH)
    ...
    def test_send_files_error(self):
        ...
        self.assertRaises(ActionHandlerError, action_handler.action, label=SEND_FILES)
    ...
    def test_send_files_invalid_user_error(self):
        ...
        action_handler = ActionHandler(**kwargs)
        action_handler.action(label=EXPORT_BATCH)
        action_handler = ActionHandler(trusted_host=False, username="bob", **kwargs)
        try:
            action_handler.action(label=SEND_FILES)
    ...
    def test_send_and_archive_files(self):
        ...
        for filename in action_handler.data.get("last_sent_files"):
            self.assertFalse(
                os.path.exists(os.path.join(app_config.outgoing_folder, filename))
            )
        for filename in action_handler.data.get("last_archived_files"):
            self.assertTrue(
                os.path.exists(os.path.join(app_config.archive_folder, filename))
            )
    ...
    def test_confirm_not_sent_raises(self):
        ...
        action_handler = ActionHandler(**kwargs)
        self.assertRaises(
            ActionHandlerError, action_handler.action, label=CONFIRM_BATCH
        )
