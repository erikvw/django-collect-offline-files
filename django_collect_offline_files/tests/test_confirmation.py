...
class TestConfirmation(TestCase):
    ...
    def setUp(self):
        ...
        tx_exporter = TransactionExporter(export_path=export_path, using=self.using)
    ...
    def test_confirmed_as_batch(self):
        ...
        confirmation = Confirmation(
            history_model=self.history.__class__, using=self.using
        )
        qs = confirmation.history_model.objects.using(self.using).filter(
            batch_id=self.history.batch_id, confirmation_code__isnull=True
        )
        ...
        try:
            confirmation.history_model.objects.using(self.using).get(
                batch_id=self.history.batch_id, confirmation_code=code
            )
    ...
    def test_confirmed_from_filename(self):
        ...
        qs = confirmation.history_model.objects.using(self.using).filter(
            filename=self.history.filename, confirmation_code__isnull=True
        )
        qs.update(sent=True, sent_datetime=get_utcnow())
