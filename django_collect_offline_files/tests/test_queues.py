...
class TestQueues(TestCase):
    ...
    def setUp(self):
        ...
        files = [f for f in os.listdir(path=self.src_path) if re.match(combined, f)]
        for f in files:
    ...
    def test_incoming_tx_queue_reload_empty(self):
        q = IncomingTransactionsFileQueue(
            src_path=self.src_path, dst_path=self.dst_path
        )
    ...
    def test_deserialize_tx_queue_task_without_tx(self):
        ...
        self.assertEqual(
            ImportedTransactionFileHistory.objects.filter(consumed=True).count(), 5
        )
    ...
    def test_deserialize_tx_queue_task_with_tx(self):
        ...
        tx_exporter = TransactionExporter(export_path=export_path, using="client")
        ...
        self.assertTrue(os.path.exists(os.path.join(import_path, batch.filename)))
        ...
        self.assertTrue(os.path.exists(os.path.join(pending_path, batch.filename)))
        ...
        self.assertEqual(TestModel.objects.filter(f1="model1").count(), 1)
