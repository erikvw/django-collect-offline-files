...
class TestFileEventHandler(TestCase):
    ...
    def test_export_import_and_move(self):
        ...
        self.assertTrue(os.path.join(app_config.incoming_folder, batch.filename))

        src_path = app_config.incoming_folder
        queue = IncomingTransactionsFileQueue(
            src_path=app_config.incoming_folder, dst_path=app_config.pending_folder
        )
        ...
        self.assertTrue(os.path.join(app_config.pending_folder, batch.filename))
        self.assertEqual(
            ImportedTransactionFileHistory.objects.filter(
                batch_id=batch.batch_id, consumed=False
            ).count(),
            1,
        )
    ...
    def test_deserialize_and_archive(self):
        ...
        self.assertTrue(os.path.join(app_config.archive_folder, batch.filename))

        self.assertEqual(
            ImportedTransactionFileHistory.objects.filter(
                batch_id=batch.batch_id, consumed=True
            ).count(),
            1,
        )
    ...
    def test_export_send_process_many(self):
        ...
        for _ in range(0, 5):
            ...
            tx_exporter = TransactionExporter(
                export_path=app_config.outgoing_folder, using="client"
            )
            batch = tx_exporter.export_batch()
        ...
        self.assertEqual(queue.unfinished_tasks, 0)
        self.assertEqual(
            ImportedTransactionFileHistory.objects.filter(consumed=False).count(), 5
        )
    ...
    def test_export_send_process_with_delete(self):
        ...
        self.send(filenames=[batch.filename], history_model=tx_exporter.history_model)
        ...
        self.assertEqual(queue.unfinished_tasks, 0)
        self.assertEqual(
            ImportedTransactionFileHistory.objects.filter(consumed=False).count(), 1
        )
    ...
    def test_deserialize_tx_queue_without_delete(self):
        ...
        incoming_queue.put(os.path.join(app_config.incoming_folder, batch.filename))
        ...
        deserialize_queue.put(os.path.join(app_config.incoming_folder, batch.filename))
