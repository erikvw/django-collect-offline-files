...
class TestTransactionExporter(TestCase):
    ...
    def test_export_file(self):
        ...
        history = tx_exporter.export_batch()
        self.assertTrue(
            os.path.exists(os.path.join(tx_exporter.path, history.filename))
        )

    def test_bad_export_file(self):
        """Assert raises when cannot create file.
        """
        tx_exporter = TransactionExporter(export_path="/blahblah", using="client")
        self.assertRaises(TransactionExporterError, tx_exporter.export_batch)
    ...
    def test_creates_history(self):
        ...
        self.assertGreater(
            tx_exporter.history_model.objects.using("client").all().count(), 0
        )
    ...
    def test_matching_batch_id(self):
        ...
        batch = tx_exporter.export_batch()
        outgoing_transactions = OutgoingTransaction.objects.using("client").filter(
            is_consumed_server=True, batch_id=batch.batch_id
        )
        for obj in outgoing_transactions:
            ...
            self.assertEqual(
                batch.history.hostname, "-".join(obj.producer.split("-")[:-1])
            )
...
class TestTransactionExporter2(TestCase):
    ...
    def test_prev_batch_id(self):
        ...
        for obj in OutgoingTransaction.objects.using("client").filter(
            ...
            self.assertEqual(batches[0].batch_id, obj.prev_batch_id)
        for obj in OutgoingTransaction.objects.using("client").filter(
            batch_id=batches[2].batch_id
        ):
            self.assertEqual(batches[1].batch_id, obj.prev_batch_id)
