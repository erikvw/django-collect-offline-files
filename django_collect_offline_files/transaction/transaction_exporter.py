...
class JSONDumpFile:
    ...
    def write(self):
        try:
            ...
            raise JSONDumpFileError(f"Unable to write to file. Got '{str(e)}'")
...
class ExportBatch:
    ...
    def open(self):
        ...
        if history_obj:
            ...
            obj = (
                ...
                .filter(is_consumed_server=True)
                .last()
            )
            prev_batch_id = obj.batch_id if obj else batch_id
        updated = (
            self.model.objects.using(self.using)
            .filter(is_consumed_server=False)
            .update(batch_id=batch_id, prev_batch_id=prev_batch_id)
        )
        if updated:
    ...
    def close(self, remote_host=None):
        ...
        self.items.update(
            is_consumed_server=True, consumer=remote_host, consumed_datetime=timestamp
        )
    ...
    def items(self):
        if self.batch_id:
            return (
                self.model.objects.using(self.using)
                .filter(batch_id=self.batch_id)
                .exclude(is_consumed_server=True)
            )
        return None
