        ...
        while True:
            try:
                ...
                message = (
                    ...
                    ' Got {}.'.format(e, self.host)
                )
                transaction_messages.add_message('error', message, permission=True)
                return False
            except socket.gaierror:
                message = (
                    'Hostname {} not known or not available'.format(self.host))
                transaction_messages.add_message('error', message, network=True)
                return False
            except ConnectionResetError as e:
                message = (
                    ' ConnectionResetError {} for {}@{}'.format(str(e), self.user, self.host))
                transaction_messages.add_message('error', message, network=True)
                return False
            except SSHException as e:
                message = ' SSHException {} for {}@{}'.format(str(e), self.user, self.host)
